CREATE TABLE IF NOT EXISTS flnav_db.http_status_codes
(
...
CREATE TABLE flnav_db.lnav_example_log
(
...
CREATE VIEW flnav_db.lnav_top_view AS
SELECT *
FROM lnav_views
WHERE name = (SELECT name FROM lnav_view_stack ORDER BY rowid DESC LIMIT 1);

CREATE TRIGGER flnav_db.lnav_top_view_update
INSTEAD OF UPDATE ON flnav_db.lnav_top_view
BEGIN
  UPDATE lnav_views
     SET top = NEW.top,
         left = NEW.left,
...
CREATE VIEW flnav_db.lnav_focused_msg AS
SELECT *,
       log_part,
       log_actual_time,
       log_idle_msecs,
...
CREATE TRIGGER flnav_db.lnav_focused_msg_update
INSTEAD OF UPDATE ON flnav_db.lnav_focused_msg
BEGIN
  UPDATE all_logs
...
CREATE VIEW flnav_db.lnav_file_demux_metadata AS
...
CREATE TABLE flnav_db.lnav_user_notifications
(
...
CREATE TABLE flnav_db.lnav_views_echo AS
SELECT name, top, "left", height, inner_height, top_time, search, selection
FROM lnav_views;

CREATE UNIQUE INDEX flnav_db.lnav_views_echo_index ON lnav_views_echo (name);

CREATE TABLE flnav_db.lnav_log_breakpoints (
